...
def genPoints(pointCount, heartScales):
    ...
    while index < pointCount:
        ...
        if heartValue < 0 and sheartValue > 0:
            ...
            rheartValue = heart_func(
                result[index][0] + 0.5, newY + 0.5, result[index][2] + 0.5, heartScales[0])
            if rheartValue > 0:
                continue
            result[index][1] = newY
            ...
            index = index + 1
            if index % 100 == 0:
                print("{ind} generated {per}%".format(
                    ind=index, per=((index / pointCount) * 100)))
...
def draw_on_buffer(screenPos, depth, draw_size):
    if draw_size == 0:
        ...
        draw_point_on_buffer(screenPos[0], screenPos[1], color_strength, depth)
        draw_point_on_buffer(
            screenPos[0] + 1, screenPos[1] + 1, color_strength, depth)
    elif draw_size == 3:
        draw_point_on_buffer(screenPos[0], screenPos[1], color_strength, depth)
        draw_point_on_buffer(
            screenPos[0] + 1, screenPos[1] + 1, color_strength, depth)
        draw_point_on_buffer(
            screenPos[0] + 1, screenPos[1], color_strength, depth)
    elif draw_size == 4:
        draw_point_on_buffer(screenPos[0], screenPos[1], color_strength, depth)
        draw_point_on_buffer(
            screenPos[0] + 1, screenPos[1], color_strength, depth)
        draw_point_on_buffer(
            screenPos[0], screenPos[1] + 1, color_strength, depth)
        draw_point_on_buffer(
            screenPos[0] + 1, screenPos[1] + 1, color_strength, depth)
...
def paint_heart(ratio, randratio, outputFile=None):
    ...
    # 生成一组随机点
    randPoints = genRandPoints(
        random_point_szie, random_scale_range, random_point_maxvar, randratio)
...
def gen_images():
    ...
    if os.path.exists("output/29.jpg"):
        return
